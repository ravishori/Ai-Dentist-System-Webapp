...
Identity architecture for M1 is recorded in **TDA-ADR-002 v1.0**. The managed identity provider is recorded in **TDA-ADR-003 v1.0** (Amazon Cognito user pools, Essentials, IdP-only). M1 implements authentication behind `AuthenticationPort`. M2 implements organization/membership/RBAC behind `AuthorizationPort`. M3 implements organization-scoped patient identity (TDA-IMP-M2-001 numbering). M4 implements organization-scoped appointments (TDA-IMP-M3-001 numbering) per approved `docs/ADR-FOLLOWUP-M4.md`. See `docs/TDA-M2-AUTHORIZATION-CONTRACT.md`, `docs/ADR-FOLLOWUP-M2.md`, `docs/TDA-M3-PATIENT-CONTRACT.md`, `docs/ADR-FOLLOWUP-M3.md`, `docs/TDA-M4-APPOINTMENT-CONTRACT.md`, and `docs/ADR-FOLLOWUP-M4.md`. Notification delivery remains unimplemented.
