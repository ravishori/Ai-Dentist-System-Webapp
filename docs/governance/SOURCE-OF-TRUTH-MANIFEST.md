...
| TDA-ADR-002         | Identity, Authentication, Authorization & Tenant Model | 1.0     | PROPOSED — partially superseded by TDA-ADR-004 for OTP IdP                                           | Security / ADR     |
| TDA-ADR-003         | Managed Identity Provider Selection                    | 1.0     | PROPOSED — partially superseded by TDA-ADR-004 (Cognito coexistence)                                 | Security / ADR     |
| TDA-ADR-004         | Passwordless OTP Identity & Self-Registration          | 1.0     | APPROVED FOR IMPLEMENTATION (2026-08-15)                                                             | Security / ADR     |
...
Identity architecture for M1 is recorded in **TDA-ADR-002 v1.0**. The managed identity provider selection for M1 Cognito OIDC is recorded in **TDA-ADR-003 v1.0**. **TDA-ADR-004 v1.0** (**APPROVED FOR IMPLEMENTATION** 2026-08-15) supersedes Cognito-as-preferred-human-IdP for C3 passwordless email/SMS OTP self-registration (invite-gated), while retaining `AuthenticationPort`, server-side `dc_session`, Cognito coexistence, Organization tenancy, and application RBAC. Schema/design notes: `docs/TDA-C3-DOMAIN-SCHEMA-DIFF.md`. See `docs/adr/TDA-ADR-004_Passwordless-OTP-Identity-and-Registration_v1.0.md` and `docs/ADR-FOLLOWUP-C3.md`. M1 implements authentication behind `AuthenticationPort`. M2 implements organization/membership/RBAC behind `AuthorizationPort`. M3 implements organization-scoped patient identity (TDA-IMP-M2-001 numbering). M4 implements organization-scoped appointments (TDA-IMP-M3-001 numbering) per approved `docs/ADR-FOLLOWUP-M4.md`. M5 implements appointment notification email delivery per approved `docs/ADR-FOLLOWUP-M5.md`. See `docs/TDA-M2-AUTHORIZATION-CONTRACT.md`, `docs/ADR-FOLLOWUP-M2.md`, `docs/TDA-M3-PATIENT-CONTRACT.md`, `docs/ADR-FOLLOWUP-M3.md`, `docs/TDA-M4-APPOINTMENT-CONTRACT.md`, `docs/ADR-FOLLOWUP-M4.md`, `docs/TDA-M5-NOTIFICATION-CONTRACT.md`, and `docs/ADR-FOLLOWUP-M5.md`.
