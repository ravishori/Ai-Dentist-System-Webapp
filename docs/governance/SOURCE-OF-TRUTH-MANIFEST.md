...
Identity architecture for M1 is recorded in **TDA-ADR-002 v1.0**. The managed identity provider is recorded in **TDA-ADR-003 v1.0** (Amazon Cognito user pools, Essentials, IdP-only). Both ADRs remain PROPOSED until human approval. Do not add vendor SDKs until TDA-ADR-003 is approved.
