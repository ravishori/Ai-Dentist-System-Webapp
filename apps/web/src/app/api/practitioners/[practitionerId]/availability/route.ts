import { handlePractitionerAvailability } from "@dentalcare/application";
...
export async function GET(
  ...
  const result = await handlePractitionerAvailability(getPractitionerService(), {
    identity: await identityFrom(),
    organizationId: requestedOrganizationId(request),
    practitionerId,
    query: new URL(request.url).searchParams,
  });
  return jsonResponse(result, "practitioner_availability");
}
